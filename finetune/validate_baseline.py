#!/usr/bin/env python3
"""
Validate vanilla Qwen 3.5 models (baseline) on the same validation dataset.
Tests if the fine-tuned improvement is real.

Usage:
    python3 finetune/validate_baseline.py
    python3 finetune/validate_baseline.py --model 2b
    python3 finetune/validate_baseline.py --save-results
"""
...
# ============================================================================
# Validation Logic (copied from validate_multiturn.py)
# ============================================================================
...
def main():
    parser = argparse.ArgumentParser(description="Validate vanilla Qwen models (baseline)")
    ...
    for model_name in models_to_test:
        print(f"\n{'='*70}")
        print(f"Testing Vanilla {model_name.upper()} (Baseline)")
        print(f"{'='*70}\n")

        model_path = VANILLA_MODELS[model_name]

        print(f"Loading vanilla {model_name} model from HuggingFace...")
        try:
        ...
        print(f"\n{'='*70}")
        print(f"Baseline Results: {model_name.upper()}")
        print(f"{'='*70}\n")
        ...
        if args.save_results:
            output_file = f"finetune/validation_baseline_{model_name}.jsonl"
    ...
    if len(results) > 1:
        print(f"\n{'='*70}")
        print("Baseline Comparison: 2B vs 4B (Vanilla)")
        print(f"{'='*70}\n")
